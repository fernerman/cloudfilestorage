...
export async function sendUpload(files, updateDownloadTask, updateTask,
    uploadTask, currPath) {
  ...
  try {
    ...
    const response = await axios.post(API_FILES, formData, {
      ...
      onUploadProgress: (progressEvent) => {
        updateTask(uploadTask, "progress",
            "Загружаем... " + bytes(progressEvent.rate) + "/c");
    ...
    if (error.response.status === 409) {
      updateTask(uploadTask, "error",
          "Файл/папка с таким именем уже существует в целевой папке!");

    } else if (error.response.status === 413) {
      const message = error.response.data?.message
          || "Превышен максимальный размер файла (ограничение 20MB).";
